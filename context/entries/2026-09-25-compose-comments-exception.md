...
- **`apps/api/scripts/compose_unchanged.py BASE..HEAD`**: exit 0 if every
  ...
  It says nothing about other condition-5 paths, and its output says so.
...
- It is not a required check. It carries the gates' crash handler, so the
  fixture harness requires a workflow to run it. It runs as its own report job,
  "Compose exception report", which is green whenever it could look, including
  when a compose file changed (a routing fact, not a defect). The harness
  cannot fixture it (it needs a git range), so it is DEFERRED with a reason,
  and it is in `test_gate_crash_exit_code.GATES`.
