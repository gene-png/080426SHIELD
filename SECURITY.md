...
| Session lifetime                | Forced re-auth after 12 h of session age everywhere (not an idle control). Access JWT 15 min and an idle bound of 15-30 min under compose only; at the config defaults (access 60 min, refresh 24 h) there is no idle bound below the 12-hour ceiling. See `docs/security.md` | Compensating controls for deferred MFA                                                   |
