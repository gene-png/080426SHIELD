...
| 5a | **#101 + #102 — persist the flags, and stop unconfirmed support scoring** | **IN PROGRESS** — migration 0044, `attack/pending.py`, run-AI + patch + heatmap + finalize + all 3 exporters + panel/matrix/rollup UI, `seed_demo` (D-055). **DONE pending merge** (PR #110). Migration 0044 + 0045, `attack/pending.py`, run-AI + patch + `confirm-citations` + heatmap + finalize + all 3 exporters + admin and CLIENT surfaces, `seed_demo` (D-055, D-056). §14 audit run: 6 findings, 5 fixed, 1 filed (#109). CI green on the branch — all six checks including the full E2E suite at 17m28s. `s33-admin-remove` passes on clean containers; `s2-management` fails LOCALLY only and is tracked as #111, not folded in here | — | merge |
