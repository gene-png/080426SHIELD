...
| 12 | **The pre-commit hook set diverges from every pinned tool version — #168** | **DONE** (PR #311, `ee819f2`, landed on `main` 2026-09-11). **Verified 2026-09-21 at `8e68d46`:** #168 is CLOSED, and all three divergences are gone from `.pre-commit-config.yaml`. The archived `mirrors-prettier` mirror is replaced by a local hook — `name: prettier (version read from pnpm-lock.yaml)`, `entry: scripts/prettier-hook.sh` — so the hook reads the same operand CI installs from, which is `CLAUDE.md`'s rule 5 applied rather than a second pin to keep in sync; the `v3.1.0` rev survives only inside a comment explaining what it was. ruff is `rev: v0.16.3` and black `rev: 26.5.1`, matching the pinned tools. **HISTORY —** found by the adversarial reviewer during the #165 sweep, **pre-existing on `main`**. `.pre-commit-config.yaml` runs prettier `v3.1.0` (46 files disagree, measured 2026-08-30), ruff `v0.6.9` against `ruff==0.16.3`, and black `24.8.0` against `black==26.5.1` — all three REWRITE, while `SECURITY.md:44` makes the hook mandatory and `docs/development.md:65` forbids `--no-verify`. A mandatory step producing the failure it exists to prevent. The prettier half needs a local hook (`mirrors-prettier` is archived and `v3.1.0` is its newest tag); the ruff/black half is a `rev` bump whose **effect is unmeasured** — black crosses two stable-style years | **Nothing logically; land it BETWEEN items** — its fix reformats 46 files, so it collides with every other item's working tree | **On start** — see below | — |


### Status correction, 2026-09-21 — what was stale, and what it cost

Items 6, 7, 8, 11 and 12 carried statuses that had stopped being true. Items 11
...
its definition.** Where the two disagree, the code decides, which is why every
status in the table above now cites a file and a quoted string rather than a
closed issue.
...
before acting on 12–18.** The items this total is built from have moved: item 7
is complete, item 6 has only F10 left of its named scope, and item 8's #123 half
has shipped. The figure below therefore over-states what remains, and by how
much is unknown from here. It is left alone on purpose:
`check_plan_totals.py` checks the arithmetic of the table against this heading,
so any status-driven edit to a row would force a new total to be invented to
keep the gate green — the gate steering an author into the defect it exists to
catch. **Re-deriving this is Gene's, bottom-up, the way item 7 was re-derived on
2026-08-27.** Until then the honest reading is "less than 12–18, and nobody has
said how much less", which is exactly why the number cannot simply be left to
speak for itself.
...
is the LEAST independent item here, not an independent one.** **#168 LANDED
2026-09-11 (`ee819f2`, PR #311), so the sequencing constraint below is spent;
recorded 2026-09-21 and kept because the reason it existed still applies to the
next reformatting change.** Fixing #168
...
**What remains: #131 and #178.** — **BOTH LANDED; nothing remains. #131 on 2026-09-06, #178 on 2026-09-09 (`6aed1b5`, PR #234). Date-qualified 2026-09-21 rather than rewritten, because the bullets below are the SIZING derivation and are the record of why the item was sized as it was.**
...
**Item 7 recorded as 1.75–3.** **#131 landed 2026-09-06**, so 0.75–1.25 of that is spent and the remaining work is #178's 1–1.75. **SUPERSEDED AS A STATUS CLAIM 2026-09-21 — #178 landed on `main` 2026-09-09 (`6aed1b5`) and item 7 is COMPLETE, so "the remaining work is #178's 1–1.75" names no remaining work.** The sentence is date-qualified rather than rewritten because it is the record of how the item was SIZED, and the number is left exactly where it was for the reason the rest of this paragraph gives. The item row, this line and the summing table still read 1.75–3: `check_plan_totals.py` verifies the parts sum to the heading, so moving any item moves the headline total, and that is a sizing decision rather than a status update.
...
- **#123 requires #114 landed.** Same file again. #123 is **item 8's
  ...
  rather than unblocked. **DISCHARGED: #114 landed 2026-09-08 and #123 itself
  landed 2026-09-10 (`6199c79`, PR #272). Recorded 2026-09-21.** Item 8's
  export/publish half is still not started and is not part of this chain.
