...
| 8 | **W6 — Risk export/publish split** | **THE #123 HALF IS DONE; THE EXPORT/PUBLISH HALF IS STILL NOT STARTED. Measured 2026-09-21 at `8e68d46`, and the halves are reported separately on purpose — "both issues closed" is not "the item is done" here, because the half that remains owns no issue.** #123 is CLOSED and landed (PR #272, `6199c79`, 2026-09-10; `routes/clients.py` now filters `Deliverable.finalized_at.is_not(None)`), and its blocker #114 is CLOSED and landed too (`routes/clients.py` resolves through `deliv.parent_version`), so the **Blocked by** cell is discharged. The export/publish split itself has shipped NOTHING: `routes/risk.py` still records export AS publication — "`export` sets `finalized_at` -- the single condition `clients.py::risk_dashboard` gates the CLIENT dashboard on -- so exporting one publishes it" — there is no publish route on the risk router, and `e2e/smoke/s30-risk-dashboard.spec.ts` still finalizes through export (`expect(exp.ok(), "export (finalize) register")`). The three costs the 2026-08-08 plan attaches to this half are therefore all outstanding: `s30`, the `DECISIONS.md` amendment reversing the export-is-release decision, and `seed_demo.py` setting `finalized_at` directly. **HISTORY — SPLIT 2026-08-30:** #123 is the `routes/clients.py` half and queues behind #114 (same file — see "File-contention dependencies"); the export/publish half is independent. Also owns **#123** — clicking Generate 404s the client's Risk dashboard with "No finalized Risk Register for your organization yet" while the finalized v1 sits right there, because the query takes the highest version with no finalized filter | **Nothing.** #114 CLOSED and landed 2026-09-08, which discharged the #123 half's only blocker; the export/publish half never had one | **1–1.5 sessions** | — |
