...
2. **Open `mvp-blocking` issues own no item**, against this section's own
   ...
   status update. **Two are now traced, so the gap is concrete rather than
   abstract: #403** (F10 — and it belongs to item 6, a mapping the 2026-08-08
   plan already made) **and #385** (a live, undisclosed wrong number on the
   CSF/ZT target pickers, which reaches item 9's bar — see that row). A rule
   violated in the abstract gets deferred; a rule violated over a live
   client-facing defect does not, which is the argument for tracing the rest. **#404 was filed by this pass and is deliberately
