...
- [x] Register a user with a brand-new, never-approved email domain → **self-registration succeeds** and the tenant is **auto-provisioned** (D-034, open self-registration); the user lands signed in on `/intake`. (s1-signup-errors.spec.ts — "a brand-new user self-registers and lands signed in on /intake"; backend `test_auth_routes.py::test_register_new_company_domain_provisions_client` / `test_register_generic_provider_gets_own_client`)
...
- [x] After **release**, the CLIENT opens the interactive ATT&CK coverage dashboard from `/documents` ("View dashboard") → KPI cards, coverage-by-tactic bar + coverage-mix donut, Detect/Prevent/Respond rings, blind-spot cards, and a searchable per-technique matrix render (D-035). (s27-attack-dashboard.spec.ts)
...
- [x] After the register is **finalized** (exported), the CLIENT opens the Risk Register dashboard via the link on `/documents` → KPI cards, the **5×5 likelihood×impact matrix**, tier-mix donut, per-axis bars, and the full register table render (D-035). (s30-risk-dashboard.spec.ts)
...
JIT provisioning. `e2e/manual/oidc-login.manual.ts` drives both paths through the
real Keycloak login form. It is a manual procedure the suite never collects
(#483): it used to be a smoke spec that self-skipped on every CI run, which read
as coverage it never provided.

- [x] Dormant by default: with the flag off, `/ready` reports `keycloak` **dormant**. (s25-admin-health.spec.ts)
...
- [ ] **Positive path** (manual: `e2e/manual/oidc-login.manual.ts`, last run green in Sprint 9 T7 per CHANGELOG, not in CI, #483) — `admin@kentro.example` (in Keycloak AND in the SHIELD DB) signs in through the real Keycloak form (`#username` / `#password`), lands authenticated, and the admin management list renders, proving the exchanged SHIELD bearer token authenticates a real API call end to end.
- [ ] **Negative path** (manual, same file and status as above) — `nolocal@atlas.example` (in Keycloak, NOT in the SHIELD DB) authenticates against Keycloak, the exchange refuses it (`oidc_no_local_account`), and `SessionExpiryGuard` signs the session out to `/sign-in?reason=oidc_exchange_failed` with the loud banner.
- [x] **Flag-off steady state** — `keycloak` reports `dormant` on `/ready`, and the credentials suite signs in green. (s25-admin-health.spec.ts asserts keycloak dormant; s0/s2 credentials sign-in)
- [ ] **Flag-off restoration** — after the flag is removed and `api`+`web` are recreated, the Keycloak provider is gone. No committed spec flips the flag or asserts the provider is absent: a manual step, filed as #565.
...
- [x] While AI is offline, the first Run AI click warns that output will be canned and offers "Load a key" / "Continue offline"; removing the key re-warns on the very next Run AI in the same session. (RunAiGuard.test.tsx, including re-warn-after-removal, the two race cases below, and one case per not-ready cause the server can report — #471. The count that stood here is deleted rather than updated: a number describing a list in another file goes stale silently, and "7" was right until #471 added cases. `s34` mints its own tenant and DRAFT assessment and asserts the Run-AI button is enabled, so it no longer self-skips on DB state — the sentence claiming it did was true before PR #19 and is corrected here.)
