...
import { CsfDashboard } from "@/components/dashboards/csf/CsfDashboard";
...
export const metadata: Metadata = { title: "NIST CSF 2.0 Dashboard" };

export default async function CsfDashboardPage({
  params,
}: {
  ...
  if (!session?.accessToken) {
    redirect(`/sign-in?callbackUrl=/dashboards/csf/${serviceId}`);
  }
  ...
  let data: CsfDashboardData | null = null;
  let notReleased = false;
  if (clientId) {
    try {
      data = await apiFetch<CsfDashboardData>(
        `/clients/${clientId}/csf/${serviceId}/dashboard`,
    ...
    return (
      ...
      >
        ...
        <p className="text-sm text-ink-secondary">
          {notReleased
            ? "This NIST CSF report hasn't been released to your organization yet. It will appear here once your SHIELD analyst releases it."
