...
def test_main_returns_2_when_the_file_cannot_be_tokenized() -> None:
    ...
    docstring promised an exit code that was unreachable -- a gate citing the
    exit convention (then as D-051; D-090 records it) while failing it. This
    pins the branch rather than the sentence.
...
def test_a_TRAILING_unknown_argument_cannot_look(tmp_path, capsys) -> None:
    ...
    """
    from scripts.check_separator_classes import main as gate_main
