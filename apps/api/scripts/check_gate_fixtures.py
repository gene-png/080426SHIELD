...
DEFERRED: dict[str, str] = {
    "merge_group_pr.py": (
        ...
        "deleted, for GitHub answering with a different PR, and for bad arguments. "
    ...
    "argument_guards.sh": (
        ...
        "IT RUNS EACH SUBJECT UNDER THE INTERPRETER ITS OWN SHEBANG NAMES, "
        "and that was a real defect found on its first CI run rather than a "
        "refinement. It invoked everything with `sh`; on Ubuntu `sh` is dash, "
        "and the three subjects declaring `#!/usr/bin/env bash` died at "
        "`set -o pipefail` before reaching any argument handling -- exit 2, no "
        "message, which is exactly what this gate flags. It could not be "
        "reproduced locally, because Git Bash's `sh` IS bash in POSIX mode and "
        "accepts the option; it took a run under real dash to see. The "
        "interpreter is not a convention to pick: measured across all seven "
        "subjects, survival under dash correlates exactly with the shebang, "
        "and `web-install-if-stale.sh` MUST stay POSIX because "
        "`docker-compose.yml` runs it as `sh /app/web-install-if-stale.sh` -- "
        "so forcing bash everywhere would stop checking the one case that "
        "matters most. A subject with no recognisable shebang is a "
    ...
    "check_mount_matches_database.py": (
        ...
        "`_contract_failures` contract is satisfied by a unit file rather than "
        "by fixtures, so this harness verifies none of it and would not notice "
        "if the file were deleted. "
        "HALF of its stated "
        "residual is now detected by the gate itself: two files in ONE tree "
        "claiming migration 0047 -- what merging two such branches produces -- "
        "exit 1 naming both files. The other half stands and is NOT covered "
        "anywhere: one file here, a different file with the same id in a tree "
        "that was never merged. `alembic_version` stores a version string and "
        "nothing about content, so nothing inside the container can tell them "
        "apart. The gate no longer claims otherwise -- its passing message says "
        "which revision is present and where it read, and does not say "
        "'agree'."
    ),
    "check_disclosure_consumers.py": (
        ...
        "now separate cases. "
        "THE DEBT RECORDED HERE IS PAID, and the sentence is corrected in "
        "place rather than deleted, because this entry's own text says a "
        "sentence is what a reader checks instead of opening the file. It "
        "read: arm 2's failing half has no case, because "
        "`AUDIT_RENDERER_EXEMPT` holds that state open until #351 lands, and "
        "when the exemption is deleted the case proving arm 2 discriminates "
        "is owed. #351 landed, the exemption is `None`, and the owed case "
        "exists TWICE -- "
        "`test_the_audit_renderer_exemption_is_DISCHARGED_and_the_slot_still_"
        "works` builds a tree with no renderer and asserts exit 1 plus the "
        "message, and `test_arm_2_needs_the_renderer_to_be_WIRED_not_merely_"
        "present` adds the wiring discriminator. A to-do list of finished "
        "items reads exactly like a real gap, and this one survived two "
        "reviews of the file it sits in. "
        "STILL TRUE: `EXEMPT_FIELDS`' expiry is covered in both directions -- "
        "a field that acquires a reader, and an entry whose field is gone -- "
        "and since #387 the WIRING is covered too, so pointing `main` at a "
        "different dict no longer passes."
    ),
    "leave_row_oracle.py": (
