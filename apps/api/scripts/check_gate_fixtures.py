...
DEFERRED: dict[str, str] = {
    ...
    "check_disclosure_consumers.py": (
        ...
        "`tests/unit/test_disclosure_consumers.py` does instead -- THIRTEEN "
        "states, every one executed. "
        ...
        "1, asserting the message), five distinct could-not-look cases each "
        "exit 2 with its own message (no repo above the start, an unparseable "
        "schema, a missing web surface, zero disclosure fields, an unknown "
        "argument), and an ADVERSARIAL case -- a field rendered ONLY in an "
        "exporter, which a reasonable person would expect to fail a gate "
        "named for screens and must not, because the rule is 'a screen OR a "
        "delivered artifact'. That case was written because applying the "
        "issue's own sketch literally would have reported a defect over "
        "`unusable_target_codes`, which reaches the client's deliverable. "
        "WHAT IS NOT COVERED, stated because the entry above certifies "
        "strength: arm 2 -- the structural check that something renders the "
        "audit `details` payload generically -- is exercised only by the tree "
        "builder handing it a renderer. There is no case asserting it FAILS "
        "when the renderer is absent, because that state is currently held "
        "open by `AUDIT_RENDERER_EXEMPT` until #351 lands. When that exemption "
        "is deleted, the case that proves arm 2 discriminates is owed."
    ),
    "leave_row_oracle.py": (
