...
**Decision 3 — coverage over assessed rows only.** `coverage_pct` = (covered + 0.5 × partial) / (covered + partial + gap). `outside_control_surface` and `unable_to_determine` are counted separately in the heatmap API response, overall and per tactic. No screen, export or deliverable renders them yet (true when written; as of 2026-09-25 #621 renders both counts on every coverage surface). They are in `scored_count`, so the catalogue total on screen does not shrink as rows move out of the denominator. **Reversed on 2026-09-25 for `unable_to_determine` (#621 round 2, finding 1):** "scored" meant two populations in two client documents, because the Risk Register's citable scope excluded unverified rows while this counted them. Both now read `coverage.UNJUDGED`, so an unverified row is not scored in either. The total on screen still does not shrink: the "Y" of "Scored: X/Y" is now `catalogue_count`, every technique tallied, not scored + unscored. On any assessment with no unverified row the two are equal, and a test pins that. This is a condition-6 change for the owner. **Gene's decision, 2026-09-25: N/A is outside the client dashboard's Detect / Prevent / Respond denominator too.** The triad then divides by the same population as the KPI row and `coverage_pct`: covered, partial and gap, not pending. Before this, the triad kept N/A in its denominator and nothing on screen said so. Gene's stated basis: no real client has a released ATT&CK report, so nothing already delivered changes. A condition-6 change.
