...
- PATCH refuses a parent's status, reason or lock with a typed 422, `parent_status_computed`. Notes stay editable. A lock is refused because a locked parent would stop following its children, which is the one thing this decision says a parent does.
...
**Condition 6.** A parent's stored status now follows its children, so coverage figures over draft and newly approved assessments can move. This comes back to the owner. **Round 2 (2026-09-25), also condition 6, the coordinator's call (option (b)) pending Gene's confirmation:** a computed parent's own stored rationale and tools are not delivered: not in the workbook, not on the client dashboard, not in the admin panel. The stored data is untouched. PATCH accepts only notes on a parent, confirm-citations refuses one, and Run AI no longer sends parents to the model. The client dashboard's Detect / Prevent / Respond triad leaves computed parents out of its population, so each technique is counted once, through its sub-techniques. This changes the triad percentages. It does not change the KPI row or `coverage_pct`, which come from the API rollup over stored statuses and never read the triad's population.
